...
- `lm`, `limma`, `lm_missing`, `lm_impute`, and `deqms` require aggregated protein-level data
...
```{r volcano_protein, fig.height=10, fig.width=12, fig.cap="Volcano plots for the protein-level facades. Rows are contrasts, columns are backends."}
...
these gaps via group-mean imputation, while `lm_impute` re-fits the lm model
after imputing individual values with the LOD and borrowing covariance from
successful fits. Both should have fewer missing proteins than plain `lm`.
