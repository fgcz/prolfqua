...
```{r eval=FALSE}
figs <- xnested %>%
  mutate(plot = map2(data, UQ(proteinIDsymbol) , plot_hierarchies_line, skylineconfig, separate = TRUE))
...
HLfigs <- xnested %>% mutate(plot = map2(data, UQ(proteinIDsymbol) , plot_hierarchies_line, skylineconfig))
...
HLfigs3 <- HLfigs3 %>% mutate(figsMed = map2(plot, medpolishPly, plot_hierarchies_add_quantline, "medpolish", skylineconfig))
