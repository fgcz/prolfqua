...
lfqtrans <- lfq$get_Transformer()$log2()$lfq
...
```{r eval=evalAll, include=TRUE}
# Not Run
fit <- proDA(se, design = ~ dilution. - 1)
contr <- list()
...
bb <- bb %>% filter(!grepl("REV__", bb$name))
bb <- bb %>% filter(!grepl("CON__", bb$name))
