...
Before the main experiment, a QC experiment is performed. We use the data from the QC experiment to estimate the within-group variance and to determine the number of sample in each group needed in the main experiment. The measurements of the main experiment are modeled using linear models, mixed-effects linear models, which can be thought of as an extension of the t-test to more than two groups.


# Aim of the meeting

The meeting aims to obtain and document all the information relevant to perform a statistically sound LFQ data analysis. 
...
We perform the data analysis according to this protocol. Changing the analysis protocol (e.g. adding contrasts or blocking factors) after the analysis was delivered might incur time delays or additional costs.


# Effect size and size of test

The parameters discussed here are already crucial for the QC experiment and will be addressed again when protein variance estimates from the QC experiment are available.

## Effect size

What is the smallest effect size, i.e., fold change in protein abundance, is of biological relevance, and you are interested in detecting? 
...
If yes, which of the group's might have the highest within-group variance? For instance, some treatment might have heterogeneous outcomes. 
...
Do you want to obtain significant p-Values for proteins with big within-group variances, or would you be satisfied with getting significances for the $X\%$ of low variance proteins? 
...
Factor name (_camel case_[^1] e.g., Treatment) : _______________________________________________________________ 



Level names (_camel case_ e.g. Control, Massage, Creme): _______________________________________________________________
...
Applies when measurements where are repeated on the same subject, e.g. over time [^3].
...
If there are more than two groups, specify hypothesis to be tested (comparison) using the factor and factor level names.
...
Example: Data normalization. Normalization is applied to remove systematic differences in protein abundance due to different sample concentrations, or different amount of sample loaded on column. Normalization is important, so that true differentially expressed proteins can be detected. 
