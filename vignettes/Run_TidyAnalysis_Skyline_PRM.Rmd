...
figs <- xnested %>% dplyr::mutate(plot = map2(data, UQ(proteinIDsymbol) , plot_hierarchies_line, skylineconfig))
...
figs3 <- intensity_summary_by_hkeys(resDataLog, skylineconfig, medpolishPly)
figs3 <- figs3()

figs3 <- figs3 %>% dplyr::mutate(plotlog = map2(data, UQ(proteinIDsymbol) , plot_hierarchies_line, skylineconfig))
plot_hierarchies_add_quantline(figs3$plotlog[[1]], figs3$medpolishPly[[1]], "medpolish", skylineconfig)

figs3 <- figs3 %>% dplyr::mutate(figsMed = map2(plotlog, medpolishPly, plot_hierarchies_add_quantline, "medpolish" , skylineconfig ))
...
protIntensity <- figs3 %>% select(table$hierarchyKeys()[1], medpolishPly) %>% 
  tidyr::unnest()
```
...
proteinIntensity <- protIntensity %>%
  dplyr::inner_join(CiRT, by= dplyr::setdiff(names(protIntensity), c("protein_Id","medpolish")), suffix = c("",".CiRT")) %>%
...
```{r}
protRez3 <- proteinIntensity %>% dplyr::group_by(protein_Id) %>% tidyr::nest()
...
```{r eval=FALSE}
protRez3 <- protRez3 %>% dplyr::mutate(plot = map2(data,protein_Id, plotProt))
...
protRez3 <- protRez3 %>%dplyr::mutate(plotMedian = map2(data,protein_Id, plotProtMedian))
