...
merged <- prolfqua::addContrastResults(prefer = contrProtMixedModerated, add = contrImp)$merged
...
ggplot(bb, aes(x = Name, y = pAUC_10)) +
  ...
  facet_wrap(~whatfix)  + 
  coord_cartesian(ylim = c(min(bb$pAUC_10),82.5)) + 
  theme_minimal() + 
...
ggplot(bb, aes(x = Name, y = pAUC_20)) +
  geom_bar(stat = "identity") +
  facet_wrap(~whatfix)  +
  coord_cartesian(ylim = c(min(bb$pAUC_20),87.5)) + 
  theme_minimal() + 
  theme(axis.text.x = element_text(angle = -90, vjust = 0.5))  
