...
prolfqua::table_facade(benchmark_ProtModerated$smc$summary, caption = "Nr of proteins with Nr of estimated contrasts.")
...
# TODO implement as proper function ...
modPmod <- contrProtModerated$get_contrasts()
modImp <- contrImp$get_contrasts()
modPmod <- filter(modPmod,!is.na(statistic))
more <- setdiff(select(modImp, protein_Id, contrast) , select(modPmod, protein_Id, contrast))
xx <- inner_join(more, modImp )
merged <- bind_rows(modPmod, xx)
