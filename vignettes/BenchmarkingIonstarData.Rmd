...
prolfqua:::.summarise_missing_contrasts(ttd$data)
...
ttd$data %>% filter(protein_Id == "sp|A0A0U1RRL7|MMPOS_HUMAN~A0A0U1RRL7")
...
lapply(allBenchmarks, function(x){x$complete(TRUE)})
bb <- (allBenchmarks$benchmark_imputation$data())
bb <- (allBenchmarks$benchmark_Prot$data())

res <- map_df(allBenchmarks, function(x){x$pAUC()})
resAll <- res %>% filter(contrast == "all")

lapply(allBenchmarks, function(x){x$complete(TRUE)})
dim(allBenchmarks$benchmark_imputation$get_confusion())

res <- map_df(allBenchmarks, function(x){x$pAUC()})
resAllB <- res %>% filter(contrast == "all")


ggplot(resAll, aes(x = Name, y = pAUC_10)) + geom_bar(stat = "identity") +
  facet_wrap(~what)  + coord_cartesian(ylim = c(60,90)) + 
  theme_minimal() + 
