...
```{r benchmarkModel1}
debug( prolfqua::ionstar_bench_preprocess)
...
lapply(allBenchmarks, function(x){x$complete(FALSE)})
...
ggplot(resAll, aes(x = Name, y = pAUC_10)) +
...
ggplot(resAll, aes(x = Name, y = pAUC_20)) +
  geom_bar(stat = "identity") +
  facet_wrap(~what)  +
