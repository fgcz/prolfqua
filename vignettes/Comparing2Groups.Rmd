...
The `fileName` is the column with the raw file names, however for labeled TMT experiments, it can be used to hold the name of the TMT channel.
...
In the MaxQuant's `proteinGroups.txt` file, we already have the data on protein level.
...
In addition, you need to describe the `factors` of the analysis, i.e., the column containing the explanatory variables.  By setting the `factorDepth` variable to $1$, we include only the "dilution." variable in the modeling.
...
This example will run only if the following R packages are installed on your machine:
