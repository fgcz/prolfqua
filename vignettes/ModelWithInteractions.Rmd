...
time point, genetic background, or other factors which you would like to check
for confounding.
...
The `AnalysisTableAnnotation` has the following fields that need to be
populated:
- fileName
- hierarchy
- factors
...
The `fileName` is the column with the raw file names, however for labelled TMT
...
Of note, the MaxQuant software encodes missing values with zero.
...
Below, the code snippet graphs the heatmap of the protein abundances and a
...
Examine proteins with a significant interaction between the two factors
treatment and batch.
