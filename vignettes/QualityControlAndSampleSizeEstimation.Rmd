...
The `hierarchy` describes the structure of the MS data. In this case we have only protein level measurements and therefore `hierarchyDepht` is equal to _one_.
...
```{r plotPCA, fig.cap="Principal component analysis for all samples", fig.align=5, fig.height=5}
pca
```
...
transformedA <- transformedA$data |> dplyr::filter(dilution.  == "a")
