...
We will use biological replicates from the _a_ dilution for the sample size
estimation. We also remove all proteins identified only by a single peptide.
...
We need to _tell_ `prolfqua` which columns in the table contain what information.
This is done using the `AnalysisTableAnnotation` class.

The `hierarchy` describes the structure of the MS data. In this case, we have
only protein level measurements.
In addition, you need to describe the factors of the analysis, i.e., the column
containing the explanatory variables.
You also need to specify the column containing the protein identifiers and the
protein intensities.

```{r analysisTableAnnotation}
...
Create the `LFQData` class instance and remove zeros from data.
...
Here we demonstrate some helper plotting functions.
...
plots number of proteins:

```{r lfqdataGet_Summariser, fig.cap="# of proteins with 0,1,...N missing values"}
lfqdata$get_Summariser()$plot_missingness_per_group()
```


plots number of proteins with x missing values:
```{r lfqdataGet_Summariser2, fig.cap="Cumulative sum of the # of proteins with 0, 1, ..., n missing values"}
lfqdata$get_Summariser()$plot_missingness_per_group_cumsum()
```

plots intensity distribution:
...
The $log_2$ stabilizes the variance, while the z-scaling removes systematic
...
plots the intensity distrubtion:
...
We can also observe at the correlation among the samples.
...
We are getting statistics for each protein in each group. The information
includes group size, number of observations, standard deviation, variance,
and mean.
...
You can also estimate the sample size needed to perform differential expression
analysis for each protein, given the `power`, `delta`, and `sig.level`.
