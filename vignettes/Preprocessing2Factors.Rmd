...
data <- dplyr::inner_join(annotation, startdata, by = "raw.file")
...
p2691 <- prolfqua::LFQData$new(adata, config)
...
p2370 <- prolfqua::LFQData$new(adata, config)
...
data <- dplyr::bind_rows(p2691$data, p2370$data)
...
```{r mergeData}
pMerged <- prolfqua::LFQData$new(data, config)
...
su$plot_hierarchy_counts_sample()
