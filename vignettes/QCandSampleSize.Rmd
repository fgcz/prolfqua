...
x <- prolfqua::hierarchy_counts(data, local_config)
...
```{r}
nrSamples <- nrow(prolfqua::table_factors(data, local_config) )
...
if (ncol(x3) >= 4) {
  ...
  ggplot2::ggplot(pltdt, ggplot2::aes(x = protein_with, y = n)) + 
    ggplot2::geom_col(color = "black", fill = "white") +
    ggplot2::geom_text(aes(label = n), nudge_y = nudgeval)
}
```
...
```{r conditional_print, child='_CVReport.RmdT', eval = show_text}
```
...
samples <- dplyr::select(dataTransformed, local_config$table$sampleName) |> dplyr::distinct() |>  dplyr::pull()
if (length(samples) > 10) {
  limit <- dplyr::select(dataTransformed, local_config$table$sampleName) |> dplyr::distinct() |>  dplyr::pull() |> sample(10)
...
ggplot2::ggplot(sampleSize$long, ggplot2::aes(x = quantile, y = N)) +
  ggplot2::geom_bar(stat = "identity", color = "black", fill = "white") +
  ggplot2::geom_text(ggplot2::aes(label = N), nudge_y = nudgeval) + 
  ggplot2::facet_wrap(~ paste0("FC==", round(FC,digits=2), "~or~log[2](FC)==", delta), 
...
prolfqua::table_facade(
  prolfqua::table_factors(data, local_config),
...
prolfqua::table_facade(
  prolfqua::hierarchy_counts_sample(dataTransformed, configuration = local_config)("wide")
  , caption = caption)
