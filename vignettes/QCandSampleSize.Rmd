...
params:
  configuration:  !r quote( prolfqua::ionstar$filtered()$config$clone(deep=TRUE) )
  project_conf: NULL
  data: !r quote( prolfqua::ionstar$filtered()$data )
  plot_density: TRUE
...
local_project_conf <- eval(params$project_conf)
...
While the overall number of `r if(params$pep){"proteins and peptides"}else{"proteins"}` can highly vary depending of the type of experiment, it is crucial that the number of `r  if(params$pep){"proteins and peptides"}else{"proteins"}`  between your biological replicates is similar (reproducibility). Table \@ref(tab:hierarchyCounts) summarizes the number of `r if(params$pep){"proteins and peptides"} else {"proteins"} ` of proteins and peptides in all samples.

```{r prep_hierarchy_counts}
...
caption <- paste0("Nr of ", if(params$pep){"proteins and peptides"} else {"proteins"} ," detected in all samples.")
dxy <- data.frame(NR = x)
```
...
```{r conditional_print, child='_CVReport.Rmd', eval = show_text}
```
...
if (!local_config$table$is_intensity_transformed) {
  data_tf <- prolfqua::transform_work_intensity(data_cc,
                                                  ...
                                                  log2)
  dataTransformed <-
    prolfqua::applyToIntensityMatrix(data_tf, local_config, robust_scale)
...
if (length(samples) > 10) {
  ...
  prolfqua::pairs_smooth( toWideConfig(ldata, local_config, as.matrix = TRUE)$data )
}else{
  prolfqua::pairs_smooth( toWideConfig(dataTransformed, local_config, as.matrix = TRUE)$data )
}
```
