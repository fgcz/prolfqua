...
tt <- quantable::split2table(names(Contrasts),split = "_")
tt <- data.frame(tt)[,2:3]
colnames(tt) <- c("ratio" , "expected fold-change")
tt <- tibble::add_column(tt, contrast =  Contrasts, .before = 1)
LFQService::table_facade(tt, caption = "All possible Contrasts given 5 E. coli dilutions of the Ionstar Dataset")
...
tt <- quantable::split2table(relevantContrasts,split = "_")
tt <- data.frame(tt)[,2:3]
colnames(tt) <- c("ratio" , "expected fold-change")
tt <- tibble::add_column(tt, contrast =  Contrasts[names(Contrasts) %in% relevantContrasts], .before = 1)
LFQService::table_facade(tt, caption = "Contrasts used for benchmark.")
...
mod <- LFQService::build_model(protLFQ$data, modelFunction)
mod$coef_pairs()$plot

```

```{r computeContrastsModerated}
contr <- LFQService::ContrastsModerated$new(mod, relevantContrasts)
...
```{r benchmark}
ttd <- LFQService::ionstar_bench_preprocess(contrdf)
...
```{r}
contrsimple <- LFQService::ContrastsSimpleImpute$new(lfqdataNormalized, relevantContrasts)

cs <- contrsimple$get_contrasts()
cs <- cs %>% rename(estimate = estimate_median)
ttd <- LFQService::ionstar_bench_preprocess(cs)
ttd$data
medpol_benchmark <- make_benchmark(ttd$data,
...
medpol_benchmark$plot_score_distribution()
medpol_benchmark$plot_ROC()
medpol_benchmark$pAUC_summaries()
