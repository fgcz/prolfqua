...
```{r}
mqdata$data <- LFQService::filter_proteins_by_peptide_count(mqdata$data, mqdata$config)$data
```
...
IonstarData <- R6::R6Class(
  ...
  public = list(
    ...
    normalized = function(){
      return(list(data = data_N, config = config_N))
    },
...
ionstar <- IonstarData$new(lfqdata$data, lfqdata$config, lfqdataNormalized$data, lfqdataNormalized$config)
