...
nested <- xx %>% group_by_at(config$table$hierarchyKeys()[1]) %>% tidyr::nest()
...
ff <- proteinIntensities %>% group_by_at(config$table$hierarchyKeys()[1]) %>% tidyr::nest(.key = "topNIntensity")
