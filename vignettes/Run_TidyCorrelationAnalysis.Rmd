...
knitr::kable(head(LFQService::getMissingStats(longQSummaries, config)))
...
plot_heatmap_cor(longQNASummaries, config)
...
nested <- xx %>% group_by_at(config$table$hierarchyKeys()[1]) %>% nest()

dim(nested$data[[1]])
linePlotHierarchy_configuration(nested$data[[1]], nested$protein_Id[[1]], config) + 
  geom_line(aes_string(linetype = "srm_decorelated"), lwd=0.2) 
linePlotHierarchy_configuration(nested$data[[2]], nested$protein_Id[[2]], config) + 
...
```{r}
plot_heatmap_cor(qvalFiltCorr, config)
```
