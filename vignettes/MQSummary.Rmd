...
knitr::kable(hierarchy_counts_sample(data, configuration = config), caption = "Number of qunatified peptides and proteins per sample.")
```

```{r sampleNameRawFileMapping}
sampleNameRawFileMapping <- data %>% dplyr::select(config$table$sampleName, config$table$fileName) %>%distinct()
knitr::kable(sampleNameRawFileMapping, caption = "Sample Name to Raw file mapping")
```
...
```{r missignessHistogram, fig.cap="Missigness patterns.", fig.width=7, fig.height=8}
...
```{r correlationHeat, fig.height = 5, fig.cap="Peptide intensity correlation between samples Heatmap."}
...
sd[sd < 0.05] <- sample(sd[sd >= 0.05], sum(sd < 0.05))
getSampleSize <- function(sd){power.t.test(delta = 1, sd=sd ,power=0.8,sig.level=0.05)$n}
N <- sapply(sd, getSampleSize)

sampN <- as.matrix(quantile(N, probs= c(.25,.5,.75,.9)))
colnames(sampN) <- c( "sampleSize")

knitr::kable(sampN, caption="Sample size needed to report a fold change greater than 2 with a significance level of 0.05 and power 0.8.")
