...
PAnnotated <- LFQServiceData::spectronautDIAData250
...
resDataLog <- transform_work_intensity(resData,config, .func =  log2)
```
...
#Fit the models
models <- fit.model(protdata = proteins,
...
for (i in 1:length(results)) {
  tmp <- LFQService::matrix_to_tibble(results[[i]])
...
LFQService::multigroupVolcano(allContrasts,
                             effect = "estimate",
                             p.value = "pval",
                             condition = "contrast",
                             xintercept = c(-1,1),
