...
We can compute the differences among the goups and test for differential expression.
`r names(grp2$Contrast)` = `r grp2$Contrast`,

using the coefficients of the linear model, estimated using linear regression.
...
(ref:imputedProt) Proteins with missing value imputation.
...
imputed_Average$get_Plotter()$raster(rownames = TRUE)
```
