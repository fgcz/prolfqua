...
Using the linear model we compare the the following two conditions and test for differential expression.
...
Figure \@ref(fig:volcanoplot) plots the $-\log_10(FDR)$ against the difference in the two conditions. Significant calls are made with FDR smaller than `r grp2$FDRthreshold` and $log_2$ fold change greater than `r grp2$log2FCthreshold` (see Figure \@ref(fig:volcanoplot)). Table \@ref(tab:nrsignificant) summarizes the number of significant calls while \@ref(tab:top20table) lists the 20 proteins with the smallest FDR. The heatmap (Figure \@ref(fig:siproteins)) shows proteins with an FDR less than `r grp2$FDRthreshold`.
...
significant <- grp2$contrMerged$contrastDF |> dplyr::filter(FDR < grp2$FDRthreshold & abs(diff) > grp2$log2FCthreshold )
...
```{r imputedProt, fig.cap = "Proteins with missing value imputation.", fig.height=10, eval = plotMissing, eval = FALSE}
