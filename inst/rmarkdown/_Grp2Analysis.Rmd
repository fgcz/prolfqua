...
cpl <- grp2$RES$contrMerged$get_Plotter(FCthreshold = grp2$Diffthreshold, FDRthreshold = grp2$FDRthreshold)
...
cpl$ma_plotly(fc = GRP2$Diffthreshold)

```

Table \@ref(tab:nrsignificant) summarizes the number of significant calls with FDR < `r grp2$FDRthreshold` and difference > `r grp2$Diffthreshold`, while \@ref(tab:top20table) lists the 20 proteins with the smallest FDR.


```{r nrsignificant, results="markup", eval=TRUE}
ctdata <- grp2$RES$contrMerged$get_contrasts()

x <- data.frame(table(abs(ctdata$diff) >  grp2$Diffthreshold  & ctdata$FDR < grp2$FDRthreshold))
...
mycap <- paste0("Number of not significant and significant proteins with difference > ", grp2$Diffthreshold, " and FDR < ", grp2$FDRthreshold, "."  )
...
significant <- ctdata |>
  dplyr::filter(.data$FDR < grp2$FDRthreshold & abs(.data$diff) > grp2$Diffthreshold )
...
cat(paste("Furthermore, figure \\@ref(fig:siproteins) shows a heatmap of proteins with the FDR < ", grp2$FDRthreshold ," and diff > ",  grp2$Diffthreshold, ".") ,
paste("Figure \\@ref(fig:boxplot) shows the boxplots of ", nrb, " significant proteins."))

```

(ref:siproteins) Heatmap showing proteins with FDR < `r grp2$FDRthreshold` and diff > `r  grp2$Diffthreshold`.
