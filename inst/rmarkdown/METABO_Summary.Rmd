---
title: "Summarize Metabolite Measurements"
...
vignette: >
  %\VignetteIndexEntry{Summarize Metabolite Measurements}
...
#xx %>% group_by_at(config$table$factorKeys()) %>% summarise(n=n())
...
knitr::kable(data.frame(NR = x), caption="Number of Compounds")
```


```{r hierarchyCountsSample }
knitr::kable(hierarchy_counts_sample(data, configuration = config), caption = "Number of quantified Compounds per sample.")

```


```{r hierarchyCountsSampleBarplot, fig.cap="Number of quantified Compounds per sample."}
...
```{r missingFigIntensityHistorgram, fig.width=7, fig.height=7, fig.cap="Top - intensity distribution of peptides with 0, 1 etc. missing values. B - number of compounds with 0,1,2 etc. mssing value."}
...
```{r intensityDistribution, fig.cap="Not normalized intensity distribution.", fig.height=4}
if(params$plot_density){
...
We apply the `vsn::justvsn` transformation to the data. This transformation transfroms and scales the data to reduce the variance.
...
```{r sdviolinplots,fig.cap="Visualization of compounds standard deviations. A) all. B) - for low (bottom 50) and high intensity compounds (top 50)."}
...
stats_res %>% group_by(!!sym(config$table$factorKeys()[1])) %>% nest() -> xx 
xx %>% mutate(sd_quantiles = map(data, ~toQuantiles(.$sd) )) %>%
...
```{r printSDTable}
knitr::kable(sd_quantiles_res, caption="CV at 0.1, 0.25, 0.5, 0.75, 0.9 quantiles.")
```



```{r plotTransformedIntensityDistributions, fig.cap="Compounds intensity distribution after transformation."}
if(params$plot_density){
...
```{r correlationHeat, fig.height = 5, fig.cap="Heatmap of Compounds intensity correlation between samples"}
plot_heatmap_cor(dataTransformed,config)
```


```{r overviewHeat, fig.cap="Sample and compounds Heatmap."}
if(params$plot_sd_vs_mean){
...
This allows us to calculate the required sample size n. The table \@ref(sampleSize) summarizes how many samples are needed to detect a fold change of $1.5,2,4$ at a significance level of $0.05$ and power of $0.8$ for $50,60,70,80, 90$ percent of the compounds.


```{r sampleSize}
sampleSize <- lfq_power_t_test_quantiles(dataTransformed, config, delta=2)
