...
tmpc <- config$clone(deep = TRUE)
tmpc$table$hierarchyLevel <-1
x3 <- summarizeHierarchy(data, tmpc)

if(tmpc$table$hierarchyLevel < length(tmpc$table$hierarchyKeys()))
{
  cnames_x3 <- config$table$hierarchyKeys()[tmpc$table$hierarchyLevel +1]
  cnames_x3 <- paste0(cnames_x3,"_n")
  x3 <- x3 %>% dplyr::mutate(protein_with = case_when(!!sym(cnames_x3) == 1 ~ "one",
                                                      !!sym(cnames_x3) > 1 ~ "two and more"))
  
  pltdt <- x3 %>% count(protein_with)
...
data_cc <- data_small_rm #LFQService::completeCases(data_small_rm, config)
...
(ref:missingnessHeatmap) Heatmap of missing peptide quantifications clustered by sample. (The number of shown peptides is limited to $10000$).
...
cv_quantiles_res <- summarize_cv_quantiles(stats_res, config, stats="CV")$wide
...
sd_quantile_res2 <- summarize_cv_quantiles(stats_res, config, stats="sd")$wide
```
