...
```{r parisplotSmooth, fig.cap = "Pairsplot of correlations.", fig.height=7, fig.width=7}
...
An important factor in estimating the sample sizes is the smallest effect size (`r params$pep_prot` fold changes) you are interested in detecting between two conditions, e.g. a reference and a treatment. Smaller biologically significant effect sizes require more samples to obtain a statistically significant result. Typical $log_2$ fold change thresholds are $0.59, 1, 2$ which correspond to a fold change of $1.5, 2, 4$.
