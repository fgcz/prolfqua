...
- Workunit: `r config$workunit_Id`
- Project: `r config$project_Id`
- Order : `r config$order_Id`
...
samples <- dplyr::select(dataTransformed, config$table$sampleName) %>% distinct() %>%  pull()
if(length(samples) > 10){
  limit <- dplyr::select(dataTransformed, config$table$sampleName) %>% distinct() %>%  pull() %>% sample(10)
  ldata <- dataTransformed %>% dplyr::filter(!!sym(config$table$sampleName) %in% limit)
  quantable::mypairsSmooth( toWideConfig(ldata, config, as.matrix = TRUE)$data )
}else{
...
```{r sampleSize}
knitr::kable( dplyr::select( sampleSize$summary, -probs), caption="Sample size needed to detect a log fold change greater than delta with a significance level of 0.05 and power 0.8 when using a t-test to compare means.", digits = 2, format = "latex")
...
paste("Number of quantified ", if(params$pep){"peptides and proteins"}else{"peptides"}, " per sample.")
