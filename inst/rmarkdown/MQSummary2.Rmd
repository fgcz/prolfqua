...
```{r hierarchyCounts}
x <- hierarchyCounts(data, local_config)
...
x3 <- summarizeHierarchy(data, local_config)
if(ncol(x3)>=4)
{
...
data_cc <- data
p <- missignessHistogram(data_cc, local_config)
xx3 <- missingPerCondition(data_cc, local_config)
xx4 <- missingPerConditionCumsum(data_cc, local_config)
...
plot_NA_heatmap(data = data_cc, config = local_config)
...
plot_heatmap_cor(dataTransformed, local_config, R2 = TRUE, col = colorRampPalette(c("white", "red"))(1024))
```
...
if(params$plot_sd_vs_mean){
  plot_heatmap(dataTransformed, local_config)
}
```
