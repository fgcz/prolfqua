...
This document summarizes the `r params$pep_prot` variability to asses the reproducibility of the biological samples and estimates the sample sizes needed for the main experiment. 

For this analysis, we used the quantitative values measured on the `r params$pep_prot` level. 
For some proteins, we observe many peptides, which might decrease the variance of the protein estimates. However, since we measure most of the proteins with only two peptides, the peptide intensities and variances are a reasonable estimate of the protein variances. 
