...
```{r echo=FALSE}
pcaplot <- file.path(pars$qc_results , paste0(pars$prefix, "_intensities_PCA_.pdf"))
heatmapcor <- file.path(pars$qc_results , paste0(pars$prefix, "_intensities_heatmap_correlation_.pdf"))
heatmap <- file.path(pars$qc_results , paste0(pars$prefix, "_intensities_heatmap_.pdf"))
intensitieswide <- file.path(pars$qc_results , paste0(pars$prefix, "_intensities_wide_.xlsx"))
intensitiessingle <- file.path(pars$qc_results , paste0(pars$prefix, "_intensities_.pdf"))
qchtml <- file.path(pars$qc_results, paste0(pars$prefix, "_intensities_qc.html"))

```


- [PCA plot](`r pcaplot`){target="_blank"} showing the samples according to the first and second principal component.
- [Heatmap based on sample correlation](`r heatmapcor`){target="_blank"}.
- [Protein Intensity heatmap](`r heatmap`){target="_blank"}.
- The protein intensities in [excel xlsx format](`r intensitieswide`){target="_blank"}.
- A [pdf file](`r intensitiessingle`){target="_blank"} containing all the peptide and protein intensities.
- A quality control report of estimated protein intensities [html](`r qchtml`){target="_blank"}.
...
histogramPVal <-   file.path(pars$modelling_dir, "Contrasts_Histogram_moderated.p.value_Model")
histogramPValAdj <- file.path(pars$modelling_dir, "Contrasts_Histogram_moderated.p.value.adjusted_Model")
volcanoPVal <- file.path(pars$modelling_dir, "Contrasts_Volcano_moderated.p.value_Model")
volcanoPValAdj <- file.path(pars$modelling_dir, "Contrasts_Volcano_moderated.p.value.adjusted_Model")
...
- A __table__ with all fold changes (column estimate) and p-Values ([excel xlsx format](`r paste0(contrastXLSX,".xlsx")`); [html](`r paste0(contrastXLSX,".html")`){target="_blank"})  for all proteins and contrasts. For proteins not quantified in one of the conditions, _pseudo_ estimates are provided.
...
- __MA plot__ ([pdf](`r paste0(maPlot,".pdf")`){target="_blank"}; [html](`r paste0(maPlot, ".html")`){target="_blank"}). The vertical axis shows the contrast estimate (_log2 FC_) while the horizontal axis shows the mean.
