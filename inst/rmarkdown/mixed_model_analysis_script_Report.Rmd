...
We preprocess the peptide intensities reported in the peptides.txt file as follows: we removed all proteins with only one identified and quantified peptide, we remove intensities equal zero, non zero intensities are log2 transformed and modified using robust z-score transformation. Afterwards, for each protein, we fitted a mixed-effects model to the peptide intensities, and fold changes and p-values are computed based on this model for each contrast [@lmerTest2017]. For a contrast, all the protein p-values are adjusted using the Benjamini and Hochberg procedure to obtain the false discovery rate (FDR) [@BenjaminiHochberg1995].
