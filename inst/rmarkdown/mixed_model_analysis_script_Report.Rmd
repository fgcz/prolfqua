...
if (qcfile("inference_figures.pdf", check = TRUE)) {
  txt_inference_fig <- paste(c(
    "- A [pdf file](", qcfile("inference_figures.pdf"), '){target="_blank"}',
    "visualizing all the peptide and protein intensities (the peptide intensities are $\\log_2$ transformed and [z-scored](https://en.wikipedia.org/wiki/Standard_score) )."
  ), collapse = "")
}
```
...
if (qcfile("boxplot.pdf", check = TRUE)) {
  txt_boxplot_fig <- paste(c(
    "- A [pdf file](", qcfile("boxplot.pdf"), '){target="_blank"}',
    "with box-plots showing the intensity distribution of each protein in all conditions."
  ), collapse = "")
}
```
