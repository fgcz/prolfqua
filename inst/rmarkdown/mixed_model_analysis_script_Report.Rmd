...
- non zero intensities are $\log_2$ transformed and modified using robust z-score transformation (using the median and median average deviation). 
...
```{r prepareBoxplot, include = FALSE prepareBoxplot, include = FALSE}
...
For each protein, we fitted a `r if(pars$model$is_mixed) {mixed_txt} else {fixed_txt}` to the peptide intensities. 
...
Fold changes and p-Values for each contrast (see next Table) are computed based on the parameter estimates of the model model.
