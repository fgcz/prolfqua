...
```{r annotation}
ftab <- flextable::flextable(pars$annotation) %>%
  autofit() %>% 
  flextable::set_caption("Sample annotations.")
ftab
...
ft_contrast <- flextable::flextable(Contrasts)
ft_contrast <- ft_contrast %>% flextable::set_caption("Contrasts (vs - versus, gv - given).")
ft_contrast
...
- For all contrasts and proteins we provide:
    ...
    - __Volcano plot of p-Values vs fold change estimate__ ([pdf](`r  paste_file(volcanoPVal,".pdf")`){target="_blank"}; [html](`r paste_file(volcanoPVal,".html")`){target="_blank"}) which on the vertical axis shows the protein $-\log_{10}(p~values)$ versus the the log2 fold changes on the horizontal axis.
