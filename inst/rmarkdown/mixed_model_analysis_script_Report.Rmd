...
paste_file <- function(..., collapse = NULL){
  x <- c(...)
  res <- paste(x, collapse = "")
  if (file.exists(res)) {
    return(res)
  }else{
    stop("File ", res, " does not exist!")
  }
}
...
- ANOVA results ([excel xlsx format](`r paste_file(anovaXLSX,".xlsx")`);[html](`r paste_file(anovaXLSX,".html")`){target="_blank"} and histogram of p-Values [pdf](`r paste_file(anovaPDF)`){target="_blank"} which shows which factors in the model are most significant.
...
- For all contrasts and proteins:
    ...
    for all proteins and contrasts. For proteins not quantified in one of the conditions, _pseudo_ estimates are provided [^2]. The columns `estimate` and `pseudo_estimate` hold the fold changes. The column `p.value.adjusted` hods the False Discovery Rate. 
    - __Histogram of p-Values__ ([pdf](`r paste_file(histogramPVal,".pdf")`){target="_blank"}) shows the distribution of the p-Values for each contrast. This plot can be used to asses the quality of you data and model (see [How to interpret p-value histogram]( http://varianceexplained.org/statistics/interpreting-pvalue-histogram/)).
