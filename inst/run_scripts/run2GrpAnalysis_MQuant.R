...
startdata <- startdata |> mutate(proteinAnnot = case_when(
  grepl("^REV_", majority.protein.ids) ~ "REV",
  grepl("^zz|^CON", majority.protein.ids) ~ "CON",
  TRUE ~ "FW"
))

distinctprotid <- startdata |>
  select(protein_Id = majProtID, fasta.headers, proteinAnnot) |>
  distinct()
...
formula_Condition <- strategy_lm(paste0(
...
top20 <- GRP2$contrResult |>
  dplyr::select(protein_Id, log2FC = diff, conf.low, conf.high, FDR) |>
  arrange(FDR) |>
  head(20)
GRP2$top20 <- top20
...
GRP2$top20confint <- ggplot(top20, aes(
  x = protein_Id, y = log2FC,
