...
if (TRUE) {
  ...
  allresultsStanf <-
    allresultsStan %>% dplyr::filter(!sapply(summary, is.null))
  allresultsStanf <-
    allresultsStan %>% select(protein_Id, summary) %>% unnest(cols = "summary")
  allresultsStan <-
    left_join(dplyr::select(allresultsStan, protein_Id), allresultsStanf)
  tmp <- preprocess(allresultsStan)
  tmp <- tmp %>% mutate(stanR.mean.estimate = abs(mean))
  tmpStanRProb <-
    ms_bench_add_FPRTPR(tmp, arrangeby = "minp", type = "probability")
  tmpStanEEstimate <-
    ms_bench_add_FPRTPR(tmp,
                        arrangeby = "stanR.mean.estimate",
                        type = "foldchange",
                        desc = TRUE)
}
...
tmpRopecaBBS <- ms_bench_add_FPRTPR(tmp,
                                    arrangeby = "beta.based.significance", type =
                                      "probability")

tmpRopecaMedianEstimate <-
  ms_bench_add_FPRTPR(tmp,
                      arrangeby = "median.estimate",
                      type = "foldchange",
                      desc = TRUE)
...
tmpMedpolishPValue <-
  ms_bench_add_FPRTPR(tmp, arrangeby = "moderated.p.value.adjusted", type =
                        "probability")
tmpMedpolishEstimate <-
  ms_bench_add_FPRTPR(tmp,
                      arrangeby = "pseudo_estimate",
                      type = "foldchange",
                      desc = TRUE)
tmpMedpolishEstimate <-
  tmpMedpolishEstimate %>% mutate(arrangeby = paste0("medpolish.", arrangeby))
...
tmpMMpValueMod <-
  ms_bench_add_FPRTPR(tmp, arrangeby = "moderated.p.value.adjusted", type =
                        "probability")
tmpMMpValueMod <-
  tmpMMpValueMod %>% mutate(arrangeby = paste0("mm.", arrangeby))

tmpMMpValue <-
  ms_bench_add_FPRTPR(tmp, arrangeby = "p.value.adjusted", type = "probability")
tmpMMMedianEstimate <-
  ms_bench_add_FPRTPR(tmp,
                      arrangeby = "pseudo_estimate",
                      type = "foldchange",
                      desc = TRUE)
tmpMMMedianEstimate <-
  tmpMMMedianEstimate %>% mutate(arrangeby = paste0("mm.", arrangeby))
