...
allBenchmarks <- readRDS("inst/Benchresults/allBenchmarkds.RDS")


#allBenchmarks <- readRDS("inst/Benchresults/allBenchmarkdsNORMAL.RDS")

benchmark_msstats <- readRDS("inst/Benchresults/benchmark_msstats.RDS")
allBenchmarks$benchmark_mssstats <- benchmark_msstats

length(allBenchmarks)
...
percentgg <- dds %>% ggplot(aes(x = name, y = percent )) + 
  ...
  geom_text(aes(label = round(percent,digits = 1)), position = position_dodge(width = 0.9), vjust = -0.25)

nrgg <- dds %>% ggplot(aes(x = name, y = nrcontrasts )) + 
  geom_bar(stat = "identity") + 
  coord_cartesian(ylim = c(min(dds$nrcontrasts),max(dds$nrcontrasts))) +
  theme(axis.text.x = element_text(angle = -90, vjust = 0.5)) +
  geom_text(aes(label=round(nrcontrasts,digits = 1)), position = position_dodge(width = 0.9), vjust = -0.25)

gridExtra::grid.arrange(percentgg, nrgg, nrow = 2)
