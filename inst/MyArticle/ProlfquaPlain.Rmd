...
allBenchmarks$benchmark_mssstats$smc$summary
...
res <- res %>% filter(Name != "imputed_same_as_lm")
...
resAllB <- res %>% dplyr::filter(contrast == "dilution_(6/4.5)_1.3(3)")


ggplot(resAllB, aes(x = Name, y = pAUC_10)) +
  geom_bar(stat = "identity") +
  facet_wrap(~whatfix)  + 
...
ggplot(res, aes(x = contrast, y = pAUC_10n, group = Name)) +
...
ddb <- dd %>% dplyr::filter(contrast == "dilution_(9/7.5)_1.2")
...
Compare number of estimated models, Compare ROC, and compare FDR vs FDP curves

How this results generalize when using a different software.
