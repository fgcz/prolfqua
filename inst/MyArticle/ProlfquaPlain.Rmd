...
tt <- tt %>% filter(contrastsName  %in% relevantContrasts)
...
From the linear and the mixed effect models, we can obtain the residual standard deviation $\sigma$, and degrees of freedom.  [@limma] discuss how to use the $\sigma$ and $df$ of all models to estimate a moderated $\tilde \sigma$ and prior degrees of freedom. This allows to moderate the t-statistics by $\tilde{t}_{pj} &= \frac{t_{pj} s_p}{\tilde{s}_p}$ and to determine the posterior degrees of freedom and finally to moderate the p-values.
