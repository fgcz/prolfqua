...
Peptide intensities are $\log_2$ transformed and robust z-score scaled but then rescaled to preserves the original range of the data using the method `robscale`. The distribution of the normalized intensities can be seen in Figure \@ref(fig:prepro) Panel A. Protein intensities are estimated using Tukey's median polish. Figure \@ref(fig:prepro) Panel B shows the peptide intensities and in black the estimated protein intensities, while the boxplot (Panel D) shows the distribution of the protein intensity estimate in each condition. Finally Panel C shows the distribution of the standard deviations in each of the conditions for all of the proteins.
...
The following code example illustrates how contrast comparing the protein intensities in the condition can be computed. First the contrasts and the linear model are specified. Afterwards the model is fitted to the data using the `build_model` function. The importance of the factors within the model can be examined using the anova function. This facilitates model selection for data within more than one explanatory variable. Finally we estimate the contrasts either from the linear model using the `Contrasts` class, or directly from the data using the `ContrastsSimpleImpute` class. Afterwards we apply t-statistic moderation using the `ContrastModerated` class The `addContrastResults` then merges both sets of contrast estimates, preferring the one obtained from the linear model. Figure \@ref(fig:exampleContrasts) Panel A shows the distribution of the p-values, while figure Panel B shows the volcano plot  for each comparsion.
...
The Benchmark functionality of prolfqua includes ROC curves and computes partial areas under those curves (pAUC) and other scores (for more details see BenchmarkingIonstarData). We use it to study how well linear, mixed effect models or p-value moderation model quantitative mass spectrometric high throughput experiments and compare it with results produced by MSstats (Choi 2014). 

Data was preprocessed by $log_2$ transforming the peptide intensities, and subsequent robust z-score transformation. No other data filtering was used than the removal of _one hit wonders_, i.e. proteins with a single peptide assignment. We did not use imputation. We fitted the mixed effect linear model to the peptide intensities. The linear model was fitted to the protein intensities inferred from the peptide intensities using the Tukey's median polish.
