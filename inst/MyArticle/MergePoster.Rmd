...
An important aspect of mass spectrometric data are missing values. Typically, problems arising from missing data are handled using imputation. The problem with imputation is, that imputed values are treated similarily to observed data in the subsequent modelling (get text from proda paper). A way to adress this is to annotate the produced models with information about missigness which is done by for instance MSStats. How missigness can modelled using the Hurdle model is described here, however to our knowledge only the R package _proDA_ models missingness directly. In prolfqua, we also do not impute the data but attempt to model missigness by inferring the pooled variance from the observed data, and model parameters (group averages) based on a prespecied prior. 
