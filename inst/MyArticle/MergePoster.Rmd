...
LFQ experiments are conducted to determine the relative protein abundance across different conditions. LFQ by Liquid-Chromatography Mass-Spectrometry  (LC-MS) is a bottom-up method: peptides are measured and then used to determine the relative abundance of the intact protein.
...
Proteins are carriers of function and structure in living cells. Hence, measuring changes in protein abundance is the subject of active research. However, identifying and quantifying intact proteins in complex samples containing hundreds of proteins is not possible using existing methods at the moment. Hence, the bottom-up mass spectrometric method is employed (see Figure \@ref(fig:bottomUP)). Proteins that are extracted from biological samples are specifically and reproducible digested into protein fragments - peptides. The peptides are first separated by their chemical and physical properties using High-Pressure Liquid Chromatography (HPLC). Afterward, they are weighted, identified, and quantified using mass spectrometric techniques, e.g., Electro-Spray-Ionization Mass Spectrometry (ESI-MS). Finally, peptide identification is achieved by fragmenting and matching the measured fragment masses to theoretical masses computed from known peptide sequences. For quantification, intact peptide ions or products of peptide ion fragmentation are counted and aggregated. Finally, the identified and quantified peptides are assigned to proteins based on protein sequence information.

Label-Free Quantification (LFQ) Proteomics Experiments enable monitoring relative abundances of thousands of proteins in biological samples. For most studies, parallel designs with a single explanatory variable are used, e.g., treatment vs. control. More recently, more complex experimental designs are implemented with increasing numbers of samples, i.e., factorial designs and longitudinal studies (time series), sometimes with repeated measurements on the same subject. The obtained data can be modeled using linear fixed, mixed, or random-effects models [@Bates2015FittingJSS]. Based on these models, tests can be applied to examine whether a factor or interactions are significant, or differences between factor levels can be estimated, and their significance tested.  Proteins can be ordered or grouped using the estimated fold-change, t-statistics, scaled p-value, or false discovery rate (FDR) and then subjected to gene set enrichment or over-representation analysis (give references) or validation experiments.


Several packages exist to model mass spectrometry LFQ experiments, e.g. Limma[@Ritchie2015] and MSStats[@Choi2014], ROPECA[@Suomi2017bEnhanced],  MSqRob [@Goeminne2016] or proDA [@bioxrvproDA2020], to mention some, all implemented in the R. Each of them has some unique features. For example, _MSStats_ infers the model from the sample annotation and generates the model formula. At the same time, _limma_ allows to specify a linear model formula and implements the empirical Bayes variance shrinkage method. The R-package _ROPECA_, performs a roll-up of peptide level fold change and p-value estimates, obtained from _limma_, to protein level estimates. Furthermore, MSqRob implements two models: robust linear models fitted to protein intensities and robust ridge regression fitted to peptide intensities and combines them with empirical Bayes variance shrinkage known from _limma_. 
When analyzing parallel designs using a single explanatory variable, we can use all packages for the statistical data analysis of the data; however, if we want to model factorial designs or repeated measurements, we can use only some of them. Table \ref@(tab:tableOverview) gives an overview of the models supported.

```{r tableOverview}
xx <- list(proDA = c("pd" = "Y", eb = "Y", md = "Y"),
triqler = c("pd" = "Y"),
...
We were inspired by the excellent R package caret, which enables us to call various machine learning methods utilizing few functions, making selecting the best ML algorithm for your problem easy. We wanted to have a similar tool for the analysis of LFQ data. However, after examining the R packages for modeling proteomics quantification data, we observed that model specification, input, and output formats differ widely and wildly. These packages are implemented in R, and all use the R linear modeling infrastructure. All of them fit some type of linear model either to peptide or protein intensities, determine contrasts among conditions, and afterward, some of them apply empirical Bayes variance shrinkage. Therefore, the revised goal was to provide a modular design, with R linear models as a core, where features such as p-value aggregation (ROPECA) or variance shrinkage could be easily enabled if required. 
...
Furthermore, the functionality of _prolfqua_ also includes methods specific to proteomics data. For example, we implemented strategies to infer protein intensities from peptide intensities: top N, Tukey's median polish, robust linear models. Furthermore, peptide or protein intensities can be scaled using the robust scaling method or transformed using _quantile_ normalization or _vsn_ from Bioconductor to remove systematic differences among samples and heteroscedasticity. In this respect, _prolfqua_ can be compared with other R packages such _msnbase_[], _DEP_ and (see BioC2021 conference) which support the entire LFQ analysis pipeline.

To benchmark analysis pipeline and modeling methods implemented within _prolfqua_ and to compare our results with those of MSstats, we use the Ionstar [reference] dataset. 
...
When imputing, we assume that there are no observations because of the limit of detection (LOD). Furthermore, we believe that the variance of the protein is constant in all the conditions.
To handle cases where we do not have any observations in one of the conditions, we impute the estimate of the condition mean by using the average of $10\%$ smallest group mean intensities in the experiment. To estimate the variance in the condition, we use the pooled variance based on all conditions: 
...
conI <- ContrastsSimpleImpute$new( lfqp, contrasts) %>% 
  ContrastsModerated$new()
tmp <- addContrastResults(prefer = contr, add = conI)
# visualize results
...
We can use various statistics to examine the alternative hypothesis $\beta \ne 0$: the contrast estimate, i.e. the $\log_2$ fold-change $\beta$, the t-statistic $\frac{\beta}{\sqrt{var(\beta)}}$, or the $p-value$ and moderated $p-value$. For each statistic and each value of the statistics we then compute a confusion matrix (see Table \@ref(tab:confusionMatrix)). From the confusion matrix we obtain measures such as true positive rate (TPR), false positive rate (FPR), false discovery proportion (FDP) or accuracy (ACC) which are given by:
...
We assessed the performance of different modelling approaches using a benchmark dataset whose ground truth was known.  However, the dataset used has several limitations. First, the design is a parallel-group design. Secondly, essential sources of variation, present in every experiment, such as the biochemical and biological, are not measured. The dataset, captures only the variance coming from the chromatography, electrospray and the mass spectrometric measurement method. While we can extrapolate some of the results obtained to more realistic datasets, we need to be careful because the observed differences between the various tested models are small.

What we can conclude is that, if we want to order the proteins according to the likelihood of being differentially regulated, for instance to perform gene enrichment analysis, the t-statistic is better suited than the fold-change estimate, especially if the expected observed effect sizes are small.
Modelling the degrees of freedom when computing the p-values might improve the inference further. However, for the benchmark dataset used, we do not observe an improvement for neither the fixed nor mixed effect models (see Figure \@ref(fig:roc2Stage) and Tables \@ref(tab:tablesMedpolishScoresNC) for fixed and Table \@ref(tab:tablesMMScoresNC) and Figure \@ref(fig:rocMM) for mixed effects).

For the fixed effect linear model, the p-value moderation as first suggested by [@Smyth2004linear] gives a consistent improvement compared to using unmoderated ones (see Table \@ref(tab:tablesMedpolishScoresNC)). However, we could not get a similar improvment for the mixed effect model, most likely because the degrees of freedom are not correctly estimated for a large proportion of models (see Table \@ref(tab:degreesOfFreedom)).

Computing first the statistics at peptide level, e.g. the t-statistics or p-value and then summarizing these statistics by using the median, produces the highest AUC scores among all the models tested (see Figure \@ref(fig:integratedPValPlot)). By using the beta distribution to model the number of peptides observed, we can further improve the AUC scores compared with the median p-value (for more details see Table \@ref(tab:tablesRopecaScoreNC)). However, the properties of beta based probabilities are not well understood, for instance they are not uniformly distributed under the null hypothesis. The FDR estimates obtained when correcting for multiple testing with the Benjamini-Hochberg method are biased (see Figure \@ref(fig:FDRvsFDP)). A step forward might be to look into alternative ways to model the number of peptides. For now, we would resort to the use of `prot_med_lm` model.

Leveraging on these computer experiments we can provide the following advices. Aggregate the peptide intensities to protein intensities using a robust regression method. Use linear models as long as they suffice to model your experimental design and the sources of variance. Use mixed effect models only if you have to, i.e. for more complicated designs (e.g. repeated measurements) because the estimates of p-values or false discovery rates have larger errors. If you use fixed effect linear models, apply p-value moderation, since this improves the p-value estimates. If you want to sort you protein lists to perform gene set enrichment analysis, use the t-statistic. Suppose we're going to dichotomise the list of proteins, e.g. to select proteins to study them further. In that case, we can use the FDR, obtained using the Benjamini-Hochberg correction, but the estimate although not entirely unrealistic might be biased.


Large flexibility when choosing models. Intuitive contrast specification. Benchmarking results comparable with other methods. 

Use of `R6` classes for programming. Once configuration for data is specified and LFQData object configured the code becomes very concise. Auto-completion support in Rstudio makes it easy for novices to create own reports.
Infrastructure will allow to easily add more modelling methods, eg. glm's for missing data modelling, or rlm's as an alternative to lm.

Easy interoperability with other packages such as protti which are also using tidy tables to represent the data.

Outlook, Improve degrees of freedom estimation for mixed effect models.


_prolfqua_ is an easy-to-use package to analyse quantitative mass spectrometric data, report results, and benchmark MS software and statistical methods. We provide more details at the website https://wolski.github.io/prolfqua/. To install:
