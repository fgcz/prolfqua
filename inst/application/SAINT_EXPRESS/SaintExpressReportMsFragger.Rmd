...
The outputs are available in b-fabric workunit: `r BFABRIC$workunitID`, in project ID `r BFABRIC$projectID` with order ID `r BFABRIC$orderID`.
...
Protein interactions will be filtered with an _SaintScore_ of `r SEP$SSthreshold` and a fold change threshold of `r SEP$FCthreshold` which corresponds to a $\log_2(SEP$FCthreshold)$ of `r log2(SEP$FCthreshold)`.
