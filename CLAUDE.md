...
Facades split by input/output hierarchy shape. `lookup_facade(name)$needs` returns one of two values:
...
Downstream dispatch convention: protein-level readers pair with `"same"` facades only; peptide-level readers pair with either.
