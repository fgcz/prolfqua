...
**Aggregated input** (same-level, protein → protein FC; `subject_Id == hierarchy_keys`) — `R/ContrastsFacades.R`:
`lm`, `rlm`, `lm_missing`, `lm_impute`, `limma`, `limma_impute`, `limma_voom`, `limma_voom_impute`, `deqms`, `deqms_voom`, `firth`, `limpa`

**Nested input** (child → parent, peptide/precursor → protein FC; `subject_Id` is strict subset of `hierarchy_keys`) — `R/ContrastsChildToParentFacades.R`:
...
- `R/tidyMS_data_setup.R` — `setup_analysis`, `complete_cases`, `sample_subset`
