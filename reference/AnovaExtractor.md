...
- `test`:
  ...
  statistical test type ("F", "Chisq", etc.)

- `col_pval`:

  p-value column name in ANOVA output
...
- [`AnovaExtractor$new()`](#method-AnovaExtractor-new)

- [`AnovaExtractor$fun()`](#method-AnovaExtractor-fun)

- [`AnovaExtractor$clone()`](#method-AnovaExtractor-clone)

------------------------------------------------------------------------

### Method `new()`

Create a new AnovaExtractor

#### Usage

    AnovaExtractor$new(test = "F")
...
Extract ANOVA table from a fitted model

#### Usage

    AnovaExtractor$fun(x)
...
data.frame with factor column and ANOVA statistics
...
#### Usage
    ...
    AnovaExtractor$clone(deep = FALSE)
