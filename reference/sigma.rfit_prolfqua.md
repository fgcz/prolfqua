...
standard deviation of an \`lm\`; it does not affect the Wald std.error
or p-value (those come from \`vcov()\`), but it is reported and used as
the per- protein variance estimate if results are passed to limma/eBayes
moderation.
...
the rank-based scale estimate \`tauhat\`.
