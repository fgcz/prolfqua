...
\`Rfit::vcov.rfit\` returns an unnamed matrix; \`compute_contrast()\`
indexes the covariance by coefficient name, so this method restores the
dimnames from \`coef()\`.

## Usage

``` r
# S3 method for class 'rfit_prolfqua'
vcov(object, ...)
```
...
the rfit covariance matrix with row/column names set to the coefficient
names.
...
#> (Intercept)        0.01023307       -0.01133785      -0.01133785
#> Speciesversicolor -0.01133785        0.02267571       0.01133785
#> Speciesvirginica  -0.01133785        0.01133785       0.02267571
```
