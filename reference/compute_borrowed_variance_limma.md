...
Extracts median sigma and df from proteins that fitted successfully (no
NA coefficients). Used by
[`build_model_limma_impute`](https://wolski.github.io/prolfqua/reference/build_model_limma_impute.md)
to replace the artificially low variance of LOD-imputed proteins.

## Usage

``` r
compute_borrowed_variance_limma(fit)
```
...
- fit:
  ...
  MArrayLM object from
  [`lmFit`](https://rdrr.io/pkg/limma/man/lmFit.html)

## Value

list with `sigma` (median residual SD) and `df` (median residual df)
from successful proteins
