...
- nr_children_col:
  ...
  optional column name for nr_children (peptide counts); NA values in
  this column are filled with 1 for imputed rows so that weighted lm
  fits do not fail
...
- on_misalign:
  ...
  With "fail", an unborrowable full covariance matrix skips the rescue
  (the model_df is returned unchanged) instead of crashing, so the
  affected proteins surface in `get_missing()`.

## Value

modified model_df with imputed models replacing failed/singular ones
