...
summarize_stats <- function(pdata, config) {
  ...
  hierarchyFactor <- pdata |>
    ...
    dplyr::summarize(
      ...
      mean = mean(!!intsym, na.rm = TRUE), .groups = "drop_last"
    ) |>
    dplyr::ungroup()
...
lfq_power_t_test_quantiles <- function(pdata,
  ...
  if (length(sd) > 0) {
    ...
    summary <- sampleSizes |>
      dplyr::select(-.data$N_exact, -.data$delta) |>
      spread(.data$FC, .data$N, sep = "=")
...
lfq_power_t_test_proteins <- function(stats_res,
  ...
  sd_delta <- purrr::map_df(delta, function(x) {
    dplyr::mutate(stats_res, delta = x)
  })

  getSampleSize <- function(sd, delta) {
    sd_threshold <- power.t.test(
      delta = delta,
      n = min.n,
      sd = NULL,
      power = power,
...
plot_stat_density_median <- function(pdata, config, stat = c("CV", "mean", "sd"), ggstat = c("density", "ecdf")) {
  ...
  p <- ggplot(top50, aes_string(x = stat, colour = "top")) +
    geom_line(stat = ggstat) +
    facet_wrap(config$table$factor_keys()[1])
  return(p)
}
