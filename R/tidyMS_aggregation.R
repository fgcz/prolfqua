...
.extractInt <- function(pdata, expression, feature, sampleName) {
  pdata <- pdata |>
    ...
    tidyr::spread(key = sampleName, value = expression) |>
    .ExtractMatrix()
  return(pdata)
}
...
#' x <- xnested$data[[1]]
#' nn <- x |>
#'   dplyr::select(setdiff(configur$table$hierarchy_keys(), configur$table$hierarchy_keys_depth())) |>
#'   dplyr::distinct() |>
#'   nrow()
#'
...
.rlm_estimate <- function(pdata, expression, feature, samples = "samples", maxIt = 20) {
  data <- pdata |>
    select_at(c(samples, feature, expression)) |>
    na.omit()
