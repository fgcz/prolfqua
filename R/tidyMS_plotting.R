...
plot_intensity_distribution_density <- function(pdata, config, legend = TRUE) {
  p <- ggplot(pdata, aes_string(
    x = config$table$get_response(),
...
#' res <- plot_hierarchies_boxplot_df(iostar$data, iostar$config)
#' res$boxplot[[1]]
#' res <- plot_hierarchies_boxplot_df(
#'   iostar$data, iostar$config,
#'   iostar$config$table$hierarchy_keys()[1]
#' )
#' res$boxplot[[1]]
#' res <- plot_hierarchies_boxplot_df(iostar$data, iostar$config,
...
plot_pca <- function(data, config, add_txt = FALSE, plotly = FALSE) {
  ...
  text <- geom_text(aes(label = !!sym(config$table$sampleName)),
    check_overlap = TRUE,
    nudge_x = 0.25,
    nudge_y = 0.25
  )
